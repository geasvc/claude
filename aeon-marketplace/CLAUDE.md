...
| `.claude-plugin/marketplace.json` | ✅ **มีจริง** — marketplace `aeon` v0.1.0 · **2 plugin: `req` v0.3.0 กับ `design` v0.6.0 (Milestone 1 ครบ + `nfr`)** | — |
| `plugins/req/**` | ✅ **มีจริง** — `req` v0.3.0 · **8 คำสั่งครบตามแบบ** (`capture` `ask` `calc` `example` `golden` `change` `check` `help`) | — |
| `plugins/design/**` | ⚠️ **Milestone 1 ครบแล้ว 2026-08-20 (S0–S4) + S4b + S10** — `design` v0.6.0 · **ใช้ได้จริง 6 คำสั่ง: `help` · `init` · `overview` · `function` · `nfr` · `status`** · อีก 9 ตัวตามสเปก §7.1 ยังไม่ได้สร้าง ติดป้ายไว้ใน `help.md` ครบทุกตัวตาม DOC-STANDARD §3.5 · `scripts/` มี `state-dir.mjs` (สำเนาของตัวเอง จงใจ — plugin ติดตั้งแยกกัน เข้าถึงสคริปต์ของ `req` ไม่ได้ · ของที่ใช้ร่วมกันคือ**โปรโตคอล** `$AEON_STATE_DIR` ไม่ใช่โค้ด) · `req-contract.mjs` (รอยต่อเดียวที่รู้จักรูปร่าง `spec.json`) · `steps.mjs` (แคตตาล็อก step ที่เดียว + `kind` — **milestone 10 ตัวเท่านั้นที่กำหนดเงื่อนไขจบ** ส่วน `status`/`check`/`trace`/`change` ไม่มีวันเสร็จ) · `trace-design.mjs` (**ผู้ผลิต `trace.design.json` รายเดียว** สร้างจาก artifact ทุกใบที่มีบนดิสก์ ไม่ใช่จากสิ่งที่คำสั่งนั้นรู้ กราฟจึงไม่ขึ้นกับลำดับการรัน) · `init.mjs` · `context.mjs` (OV1–OV8 + หัวข้อ 1–2 + DFD 0) · `functions.mjs` (FU1–FU8 + หัวข้อ 3 + แผนภาพ use case/สถานะ) · `nfr.mjs` (NF0–NF6 + หัวข้อ 4 · **ตัวเลขที่วัดได้เท่านั้น V9**) · `status.mjs` (เงื่อนไขจบของลูป · deterministic · ห้ามคืน 0 ขณะมี stale หรือคำถามที่ blocks ไม่ว่าง) · fixture 16 ชุด · **สเปกถูกแก้ให้ตรงของจริง 5 จุดระหว่างทาง (§3.1 · §5.1 · §11.1 · §6.1 RULE→BR · §6.1 NFR→req) ทุกจุดเจ้าของเคาะ** · ⚠️ **ยังไม่มีคำสั่งไหนเขียน `done` กลับลง `design.state.json`** (§7.2 กติกา 6) — รัน `--write` สำเร็จแล้ว `status` ยังขึ้นว่า READY อยู่ · คืบหน้าอ่านที่ `docs/progress.json` task 15 |
