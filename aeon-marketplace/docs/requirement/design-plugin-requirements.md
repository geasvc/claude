...
| `UC-###` | Use Case | design |
| `NFR-###` | Non-Functional Requirement | **req** — ดูหมายเหตุข้างล่าง (แก้ 2026-08-20) |
...
> **`NFR-###` เป็นของ `req` — แก้ 2026-08-20 (เจ้าของเคาะ)** แถวนี้เดิมยกให้ design แต่ id นี้ถูกจองไปแล้ว: schema ที่ประกาศไว้ของ `req` (`schemas/spec.schema.json`) นิยาม `requirements[].nfr[]` ไว้พร้อม `"pattern": "^NFR-[a-z0-9-]+-[0-9]{3}$"` และ `kind` ที่เป็น enum ปิด · โปรเจกต์จริงมีของอยู่แล้ว — aeon-miniloan มี 10 ข้อ
>
> เป็นรอยต่อเดียวกับ `RULE-###` / `BR-xxx@vN` ข้างบน และปิดด้วยวิธีเดียวกัน: `/design:nfr` **อ้าง** id ของ req เท่านั้น ไม่มินต์ใหม่ · ด่าน NF2 ตีตกทันทีที่เจอ id ที่ req ไม่มี
>
> **สิ่งที่ design เป็นเจ้าของคือตัวเลข ไม่ใช่ id** — `req` ให้ `statement` + `kind` + `verified_by` มา แต่ไม่มีค่าที่วัดได้ ขณะที่ DoD ใน §7.1 กับ V9 บังคับให้ต้องมี · **เจ้าของเคาะ 2026-08-20 ให้ design ร่างตัวเลขกับเจ้าของ** เหมือนที่ `/design:function` ร่าง FN/UC: §1.2 ห้าม design **เก็บ requirement ใหม่** และการใส่ตัวเลขให้ข้อความที่ req เก็บมาแล้วไม่ใช่ requirement ใหม่
>
> จึง **ไม่มีสถานะ "ยังไม่ได้วัด"** โดยตั้งใจ — ไม่มีตัวเลข = error ไม่ใช่คำเตือนที่พาต่อไปได้ เพราะ "ระบบต้องเร็ว" ที่หลุดไปถึงเอกสารลูกค้าคือสิ่งที่ V9 มีไว้กันพอดี
>
> และ design ไม่เขียนซ้ำของที่ req ถืออยู่แล้ว — REQ ที่เป็นเจ้าของ · `kind` · วิธีพิสูจน์ตั้งต้น อ่านจาก output ของ req ทั้งหมด · เขียนซ้ำ = มีสองที่ให้ผิดแต่มีที่เดียวที่ถูกตรวจ (NF5 กับ NF6 ตีตกความขัดกัน แทนที่จะทับเงียบ ๆ)
